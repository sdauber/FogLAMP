			   ...
			   bool skipSentDataTypes)
{
	...
	string jsonOut = compress_string(jsonData.str());

	std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
	auto usecs = std::chrono::duration_cast<std::chrono::microseconds>( t2 - t1 ).count();
	if (usecs > 20000)
		Logger::getLogger()->info("OMF::sendToServer(): OMF formatting + compression took=%lld usecs for %d readings", usecs, readings.size());
	...
	try
	{
		...
		std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
		auto usecs = std::chrono::duration_cast<std::chrono::microseconds>( t2 - t1 ).count();
		Logger::getLogger()->info("OMF::sendToServer(): HTTP request-response took %lld usecs for a buffer size of %d bytes (compressed) (= %d bytes uncompressed)", usecs, jsonOut.size(), jsonData.str().size());
